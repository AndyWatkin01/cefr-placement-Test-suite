...
window.onload = function () {
  ...
  let rawLevel = placementDossier.listeningLevelReached || "B2";
  currentLevel = rawLevel;

  console.log(
    `Handshake Successful: Starting at ${currentLevel} for ${currentAudience}`,
  );
};
...
function handleLevelDown() {
  ...
  // Entry Point Drop logic from your Mind Map
  const isFirstAttempt = grammarSessionLog.every(
    (log) => log.subLevel === currentFull,
  );

  if (isFirstAttempt) {
    if (currentLevel.startsWith("C")) {
      transitionToState("B2.1");
    } else if (currentLevel.startsWith("B")) {
      transitionToState("A2-");
    } else if (currentLevel.startsWith("A2")) {
      transitionToState("A1-");
    } else {
      finalizeTest();
    }
  } else {
...
function finalizeTest() {
  ...
  placementDossier.finalGrammarLevel = currentLevel;
